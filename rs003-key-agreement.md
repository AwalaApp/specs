...
- There is no central server that can provide certificates or public keys for any node in Relaynet, but that is not necessary because peers always have each other's certificates. For example, a client-side application will be distributed with the certificate of the server-side endpoint, and similarly, private gateways could be distributed with the certificate of their relaying gateway (or provided by a trusted courier in a cargo relay connection).
