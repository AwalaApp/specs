...
### Ephemeral key identifiers

To ease implementation, all key identifiers MUST be represented as 32-bit unsigned integers when serialized in a X.509 certificate or a CMS EnvelopedData value. This theoretically limits the number of key pairs that can ever be used in a single session to 2 ^ 32 - 1.
