...
- DH variant: DH with 2048-bit groups (_DH-2048_), DH with 3072-bit groups (_DH-3072_), DH with 4096-bit groups (_DH-4096_), ECDH with X25519 (_ECDH-X25519_) or ECDH with X448 (_ECDH-X448_).
- Cipher: AES-128, AES-192 or AES-256.
- Hash: SHA-256, SHA-384 or SHA-512.
- Channel id: An ASCII string identifying the channel.

For example, a Relaynet service may configure its endpoints to use DH-2048 as the DH variant, AES-128 as the cipher, SHA-256 as the hash function and `My Service` as the channel id.
...
When using the [Cryptographic Message Syntax (CMS)](https://tools.ietf.org/html/rfc5652) to encapsulate ciphertext encrypted using these ephemeral keys, an enveloped-data content type MUST be used with a [`RecipientInfo`](https://tools.ietf.org/html/rfc5652#section-6.2) that uses the [`KeyAgreeRecipientInfo`](https://tools.ietf.org/html/rfc5652#section-6.2.2) choice with the following attributes:
...
- `keyEncryptionAlgorithm`: The [Key Wrap](https://en.wikipedia.org/wiki/Key_Wrap) variant of the chosen cipher ([id-aes128-wrap](http://oid-info.com/get/2.16.840.1.101.3.4.1.5), [id-aes192-wrap](http://oid-info.com/get/2.16.840.1.101.3.4.1.25) or [id-aes256-wrap](http://oid-info.com/get/2.16.840.1.101.3.4.1.45)).
...
The `encryptedContentInfo` of the `EnvelopedData` sequence MUST be encrypted and serialized according to the chosen AES variant.
