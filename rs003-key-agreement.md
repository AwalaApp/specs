...
- `subjectPublicKeyInfo`:
   - `algorithm` is set to a valid key exchange algorithm per [RS-018](rs018-algorithms.md). If the algorithm is DH (OID `1.2.840.113549.1.3.1`), then `algorithmParameters` MUST be set to `PKIKeyAgreementDHParams` as defined in the ASN.1 code below:
     ```
...
- `recipientEncryptedKeys`: A single-item sequence using the choice `KeyAgreeRecipientIdentifier` (for keys generated by key agreement protocols), which will in turn use the choice `rKeyId` to identify the recipient's ephemeral key (K<sub>y,n</sub><sup>id</sup>). The `subjectKeyIdentifier` in `rKeyId` MUST match that specified by the recipient Y in the `originator` attribute of a previous CMS enveloped data value.
...
The constraints above apply to the payload of RAMF messages, since they use CMS enveloped data values.
